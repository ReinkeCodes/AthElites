...
{#if showDraftModal && activeDraft}
  ...
  >
    <div style="background: white; border-radius: 8px; padding: 20px; max-width: 90%; width: 340px; box-shadow: 0 4px 20px rgba(0,0,0,0.3);">
      ...
      <p style="margin: 0 0 {isStale ? '10px' : '20px'} 0; color: #666; font-size: 0.9em;">We found an unfinished workout: <strong>{getDraftLabel()}</strong>. You can resume it, or discard it to start a new session.</p>
      {#if isStale}
      ...
      <div style="display: flex; gap: 10px; justify-content: flex-end;">
        <button onclick={discardAndProceed} style="padding: 8px 16px; background: {isStale ? '#d32f2f' : '#fff'}; color: {isStale ? 'white' : '#d32f2f'}; border: 1px solid #d32f2f; border-radius: 4px; cursor: pointer;">Discard</button>
